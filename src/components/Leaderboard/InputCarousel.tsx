...
const InputCarousel = ({ setNameHook }: InputCarouselProps) => {
  ...
  const handleKeyPress = (e: React.KeyboardEvent) => {
    ...
    switch (e.key) {
      case "8":
        ...
        break;
      case "2":
        i[curFocus] = (i[curFocus] + 1) % ARRAY_LENGTH;
        break;
      case "4":
        if (curFocus != 0) {
      ...
      case "Enter":
        if (curFocus === 2) {
          setNameHook(
            `${alphabet[indices[0]]}${alphabet[indices[1]]}${
              alphabet[indices[2]]
            }`
          );
        } else {
  ...
  const handleFocusOut = () => {
    setNameHook(
      `${alphabet[indices[0]]}${alphabet[indices[1]]}${alphabet[indices[2]]}`
    );
  };
